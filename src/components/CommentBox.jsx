      ...
      {open && (
        <div className="mt-2">
          <textarea
            ...
            rows={3}
            className="w-full px-3 py-2 text-sm border border-slate-300 rounded focus:outline-none focus:ring-2 focus:ring-sky-500 resize-y"
          />
          <div className="flex items-center gap-2 mt-2">
            ...
            {hasNote && (
              <button onClick={clear} className="px-3 py-1.5 text-xs font-semibold bg-slate-200 text-slate-700 rounded hover:bg-slate-300">
                Clear
              </button>
            )}
            {savedTs && !dirty && <span className="text-xs text-slate-500 ml-auto">saved {timeAgo(savedTs)}</span>}
