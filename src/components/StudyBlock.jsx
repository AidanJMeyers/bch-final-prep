...
export default function StudyBlock({ examId, sectionId, block, onStateChange }) {
  const prefix = `bchfinal:e${examId}:s${sectionId}:b${block.id}`;
  ...
  return (
    <section id={`block-${sectionId}-${block.id}`} className="scroll-mt-24 bg-white border border-slate-200 rounded-xl shadow-sm mb-6">
      ...
      <div className="px-5 py-5 prose-tight text-slate-800 leading-relaxed">
        {block.content}
      </div>
