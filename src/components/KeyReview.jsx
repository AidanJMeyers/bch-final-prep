...
export default function KeyReview({ keyReview }) {
  if (!keyReview) return <div className="bg-white border border-slate-200 rounded-xl p-8 text-center text-slate-500">No key review for this exam yet.</div>;
  const { themes = [], vocab = [], laws = [], methods = [], diagrams = [] } = keyReview;
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-violet-600 to-sky-600 text-white rounded-xl p-5">
        ...
        <p className="text-violet-100 text-sm mt-1">Big themes, vocabulary, rules, and diagrams for one last pass before the exam.</p>
      </div>

      {themes.length > 0 && (
        <Card title="🌟 Big Themes the Final Will Test">
          <ul className="space-y-3">
            {themes.map((t, i) => (
              <li key={i} className="border-l-4 border-violet-500 pl-3 py-1">
                <div className="font-semibold text-slate-900">{t.title}</div>
                <div className="text-sm text-slate-700">{t.desc}</div>
              </li>
            ))}
          </ul>
        </Card>
      ...
      {laws.length > 0 && (
        <Card title="Laws, Rules & Principles">
          <ul className="space-y-3">
            {laws.map((l, i) => (
              <li key={i} className="border-l-4 border-sky-400 pl-3 py-1">
